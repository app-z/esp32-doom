...
void IRAM_ATTR updateTask(void *arg) 
{
  ...
  while(1)
  {
    ...
    flag_audio++;
    //if (flag_audio>5){
    //	printf("Audio losted more than 5!!!");
    //}
...
// Interrupt handler.
void I_HandleSoundTask( void *ignore )
{
  // Debug.
...
*/
  ...
  { 
    ...
    if (!S_sfx[i].link)
    {
      ...
      printf("Load data from WAD file %s \n", S_sfx[i].name);
      S_sfx[i].data = getsfx( S_sfx[i].name, &lengths[i] );
    }	
    else
    {
      // Previously loaded already?
      printf("Previously loaded already?");
